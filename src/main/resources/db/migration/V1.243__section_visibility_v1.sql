...
CREATE TABLE IF NOT EXISTS section_visibility (
    ...
    updated_by varchar,
    is_deleted boolean DEFAULT false,
    UNIQUE (branch_code, mode, direction) -- Unique constraint
);
...
BEGIN
    IF NOT EXISTS (
        ...
        WHERE tablename = 'section_visibility'
        AND indexname = 'idx_section_visibility_branch_code'
    ) THEN
        CREATE INDEX idx_section_visibility_branch_code
        ON section_visibility (branch_code);
    END IF;
END $$;
