...
public interface IConsolidationRepository extends MultiTenancyRepository<ConsolidationDetails> {
    ...
    @Query(value = "SELECT id FROM consolidation_details WHERE is_migrated_to_v3 = ?1 and tenant_id = ?2 and is_deleted = false", nativeQuery = true)
    List<Long> findAllByIsMigratedToV3(boolean isMigratedToV3, Integer tenantId);
