...
public class CustomerBookingController {
    ...
    @PostMapping(CustomerBookingConstants.LIST_CONTRACT)
    @ApiResponses(value = {
            @ApiResponse(code = 200, message = CustomerBookingConstants.CONTRACT_LIST_SUCCESSFUL),
            @ApiResponse(code = 404, message = Constants.NO_DATA, response = RunnerResponse.class)
    })
    public ResponseEntity<?> platformCreateBooking(@RequestBody @Valid ListContractRequest request) {
        String responseMsg;
        ...
        try {
            ...
            responseMsg = e.getMessage() != null ? e.getMessage()
                    : CustomerBookingConstants.CONTRACT_LIST_FAILED;
            log.error(responseMsg, e);
        }
        return ResponseHelper.buildFailedResponse(responseMsg);
    }
