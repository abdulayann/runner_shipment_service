...
public class ShipmentController {
    ...
    public ResponseEntity<IRunnerResponse> completeRetrieveById(@ApiParam(value = ShipmentConstants.SHIPMENT_ID, required = true) @RequestParam Long id, @RequestParam(name = "includeColumns", required = false) List<String> includeColumns) throws ExecutionException, InterruptedException {
    ...
    public ResponseEntity<IRunnerResponse> createV1Shipment(@RequestBody @Valid ShipmentRequest request) {
        ...
        try {
            ShipmentRequest req = jsonHelper.convertValue(request, ShipmentRequest.class);
            return shipmentService.completeV1ShipmentCreateAndUpdate(CommonRequestModel.buildRequest(req), new HashMap<>(), null, false, null);
        } catch (Exception e) {
            responseMsg = e.getMessage() != null ? e.getMessage()
                    : DaoConstants.DAO_GENERIC_CREATE_EXCEPTION_MSG;
            log.error(responseMsg, e);
