...
public class CargoManifestReport extends IReport{
    ...
    public Map<String, Object> populateDictionary(IDocumentModel documentModel) {
        ...
        dictionary.put(SHIPPED_ONBOARD_TEXT, cargoManifestModel.shipmentDetails.getAdditionalDetails().getShippedOnboardText().toUpperCase());
        dictionary.put(SHIPPED_ONBOARD_DATE_DDMMMYYYY, convertToDPWDateFormat(
                cargoManifestModel.shipmentDetails.getAdditionalDetails().getShippedOnboardDate(), "ddMMMyyyy".toUpperCase(), false));
    ...
    private void processValuesContainer(V1TenantSettingsResponse v1TenantSettingsResponse, List<Map<String, Object>> valuesContainer) {
        for (Map<String, Object> v : valuesContainer) {
            ...
            updateValue(v, ReportConstants.GROSS_VOLUME, convertToWeightNumberFormat(v.get(ReportConstants.GROSS_VOLUME), v1TenantSettingsResponse));
            updateValue(v, ReportConstants.GROSS_WEIGHT, convertToWeightNumberFormat(v.get(ReportConstants.GROSS_WEIGHT), v1TenantSettingsResponse));
