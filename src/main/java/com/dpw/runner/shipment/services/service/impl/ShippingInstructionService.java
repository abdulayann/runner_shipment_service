...
public class ShippingInstructionService implements IShippingInstructionService {
    ...
    private final Map<String, RunnerEntityMapping> tableNames = Map.ofEntries(
            ...
            Map.entry("status", RunnerEntityMapping.builder().tableName(Constants.SHIPPING_INSTRUCTION).dataType(BookingStatus.class).fieldName("status").build()),
    ...
    private void afterSave(@NotNull ShippingInstruction shippingInstruction, @NotNull ShippingInstructionRequest request, Boolean isCreate) throws RunnerException {
    ...
    @NotNull
    private List<IRunnerResponse> convertEntityListToDtoList(@NotNull List<ShippingInstruction> lst) {
    ...
    private CompletableFuture<ResponseEntity<IRunnerResponse>> addAllMasterDataInSingleCall(ShippingInstructionResponse shippingInstructionResponse) {
        try {
            ...
            List<MasterListRequest> listRequests = new ArrayList<>(masterDataUtils.createInBulkMasterListRequest(shippingInstructionResponse, ShippingInstruction.class, fieldNameKeyMap, ShippingInstruction.class.getSimpleName() ));
            if (!Objects.isNull(shippingInstructionResponse.getContainersList()))
                shippingInstructionResponse.getContainersList().forEach(c -> listRequests.addAll(masterDataUtils.createInBulkMasterListRequest(c, Containers.class, fieldNameKeyMap, Containers.class.getSimpleName() + c.getId() )));
            if (!Objects.isNull(shippingInstructionResponse.getPackingList()))
                shippingInstructionResponse.getPackingList().forEach(c -> listRequests.addAll(masterDataUtils.createInBulkMasterListRequest(c, Packing.class, fieldNameKeyMap, Packing.class.getSimpleName() + c.getId() )));
            ...
            // Postprocessing
            shippingInstructionResponse.setMasterData(masterDataUtils.setMasterData(fieldNameKeyMap.get(CustomerBooking.class.getSimpleName()), CacheConstants.MASTER_LIST, true));

            if (!Objects.isNull(shippingInstructionResponse.getContainersList()))
                shippingInstructionResponse.getContainersList().forEach(c -> c.setMasterData(masterDataUtils.setMasterData(fieldNameKeyMap.get(Containers.class.getSimpleName() + c.getId() ), CacheConstants.MASTER_LIST, true)));
            if (!Objects.isNull(shippingInstructionResponse.getPackingList()))
                shippingInstructionResponse.getPackingList().forEach(c -> c.setMasterData(masterDataUtils.setMasterData(fieldNameKeyMap.get(Packing.class.getSimpleName() + c.getId() ), CacheConstants.MASTER_LIST, true)));
    ...
    private CompletableFuture<ResponseEntity<IRunnerResponse>> addAllLocationDataInSingleCall(ShippingInstructionResponse shippingInstructionResponse) {
        try {
            ...
            if (!Objects.isNull(shippingInstructionResponse.getCarrierDetails()))
                locationCodes.addAll((masterDataUtils.createInBulkUnLocationsRequest(shippingInstructionResponse.getCarrierDetails(), CarrierDetails.class, fieldNameKeyMap, CarrierDetails.class.getSimpleName() )));
            ...
            // Postprocessing
            if (!Objects.isNull(shippingInstructionResponse.getCarrierDetails()))
                shippingInstructionResponse.getCarrierDetails().setUnlocationData(masterDataUtils.setMasterData(fieldNameKeyMap.get(CarrierDetails.class.getSimpleName()), CacheConstants.UNLOCATIONS, false));
    ...
    private CompletableFuture<ResponseEntity<IRunnerResponse>> addAllContainerTypesInSingleCall(ShippingInstructionResponse shippingInstructionResponse) {
        try {
            ...
            if (!Objects.isNull(shippingInstructionResponse.getContainersList()))
                shippingInstructionResponse.getContainersList().forEach(r -> containerTypes.addAll(masterDataUtils.createInBulkContainerTypeRequest(r, Containers.class, fieldNameKeyMap, Containers.class.getSimpleName() + r.getId() )));
            ...
            if (!Objects.isNull(shippingInstructionResponse.getContainersList()))
                shippingInstructionResponse.getContainersList().forEach(r -> r.setContainerCodeData(masterDataUtils.setMasterData(fieldNameKeyMap.get(Containers.class.getSimpleName() + r.getId()), CacheConstants.CONTAINER_TYPE, false)));
    ...
    private CompletableFuture<ResponseEntity<IRunnerResponse>> addAllVesselDataInSingleCall(ShippingInstructionResponse shippingInstructionResponse) {
        try {
            if (!Objects.isNull(shippingInstructionResponse.getCarrierDetails())) {
                Map<String, Map<String, String>> fieldNameKeyMap = new HashMap<>();
                List<String> vesselList = new ArrayList<>(masterDataUtils.createInBulkVesselsRequest(shippingInstructionResponse.getCarrierDetails(), CarrierDetails.class, fieldNameKeyMap, CarrierDetails.class.getSimpleName()));
                Map v1Data = masterDataUtils.fetchInBulkVessels(vesselList);
                masterDataUtils.pushToCache(v1Data, CacheConstants.VESSELS);
                shippingInstructionResponse.getCarrierDetails().setVesselsMasterData(masterDataUtils.setMasterData(fieldNameKeyMap.get(CarrierDetails.class.getSimpleName()), CacheConstants.VESSELS, false));
            }
    ...
    private CompletableFuture<ResponseEntity<IRunnerResponse>> addAllCarrierDataInSingleCall(ShippingInstructionResponse shippingInstructionResponse) {
        try {
            if (!Objects.isNull(shippingInstructionResponse.getCarrierDetails())) {
                Map<String, Map<String, String>> fieldNameKeyMap = new HashMap<>();
                List<String> vesselList = new ArrayList<>(masterDataUtils.createInBulkCarriersRequest(shippingInstructionResponse.getCarrierDetails(), CarrierDetails.class, fieldNameKeyMap, CarrierDetails.class.getSimpleName()));
                Map v1Data = masterDataUtils.fetchInBulkCarriers(vesselList);
                masterDataUtils.pushToCache(v1Data, CacheConstants.CARRIER);
                shippingInstructionResponse.getCarrierDetails().setCarrierMasterData(masterDataUtils.setMasterData(fieldNameKeyMap.get(CarrierDetails.class.getSimpleName()), CacheConstants.CARRIER, false));
            }
            return CompletableFuture.completedFuture(ResponseHelper.buildSuccessResponse(Arrays.asList()));
        }  catch (Exception ex) {
