package com.dpw.runner.shipment.services.migration.entity;

import com.dpw.runner.shipment.services.aspects.MultitenancyAspect.MultiTenancy;
import lombok.AllArgsConstructor;
import lombok.Getter;
import lombok.NoArgsConstructor;
import lombok.Setter;

import javax.persistence.Column;
import javax.persistence.Entity;
import javax.persistence.Table;
import java.time.Instant;
import java.util.UUID;

@Entity
@Setter
@Getter
@NoArgsConstructor
@AllArgsConstructor
@Table(name = "network_transfer_backup")
public class NetworkTransferBackupEntity extends MultiTenancy {

    @Column(name = "network_transfer_id")
    private Long networkTransferId;

    @Column(name = "network_transfer_guid")
    private UUID networkTransferGuid;

    @Column(name = "network_transfer_details", columnDefinition = "jsonb")
    private String networkTransferDetails;

    @Column(name = "back_up_time_utc", nullable = false)
    private Instant backUpTimeUtc = Instant.now();

}
