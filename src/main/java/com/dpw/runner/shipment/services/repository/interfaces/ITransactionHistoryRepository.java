...
public interface ITransactionHistoryRepository extends MultiTenancyRepository<TransactionHistory> {
    ...
    @Query(value = "SELECT * FROM transaction_history th WHERE th.entity_id = :entityId AND th.entity_type = :entityType AND th.tenant_id = :tenantId AND th.is_deleted = false order by created_at desc", nativeQuery = true)
