...
//        dictionary[ReportConstants.AGENT] = tenantDetails;
        ...
        if(shipmentInfo != null){
            ...
            if(routingInfoRows != null && routingInfoRows.size() > 0){
                ...
                flightNumberList.add(String.format("%s %s", dictionary.get(ReportConstants.BY_FIRST), dictionary.get(ReportConstants.FLIGHT_NO1)));
...
//                    carrierRow = fetchCarrier(carrierSet);
                    ...
                    flightNumberList.add(String.format("%s %s", dictionary.get(ReportConstants.BY_SECOND), dictionary.get(ReportConstants.FLIGHT_NO2)));
...
//                    carrierRow = fetchCarrier(carrierSet);
                    ...
                    flightNumberList.add(String.format("%s %s", dictionary.get(ReportConstants.BY_THIRD), dictionary.get(ReportConstants.FLIGHT_NO3)));
    ...
    public static String getStringFromDict(Map<String,BigDecimal> chargeDict)
    {
        ...
        for(String charge : chargeDict.keySet())
        {
            if(chargesStr.isEmpty()) {
                chargesStr.append(charge).append(":").append(IReport.addCommas(chargeDict.get(charge)));
            } else {
                chargesStr.append(" , ").append(charge).append(":").append(IReport.addCommas(chargeDict.get(charge)));
            }
        }
