            ...
            List<PackingRequestV2> res = packingList.stream()
                    .map(item -> {
                        ...
                        if (shipmentGuid == null && item.getShipmentId() != null) {
                            try {
                                p.setShipmentGuid(shipmentDao.findById(p.getShipmentId()).get().getGuid());
                        ...
                        if (consoleGuid == null && item.getConsolidationId() != null) {
                            try {
                                p.setConsolidationGuid(consolidationDetailsDao.findById(p.getConsolidationId()).get().getGuid());
                            } catch (Exception ignored) {
                            }
