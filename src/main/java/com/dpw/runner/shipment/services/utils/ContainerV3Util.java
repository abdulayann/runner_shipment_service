...
//        try {
//            log.info("Starting downloadContainers with request: {}", request);
//
//            List<ShipmentsContainersMapping> mappings;
//            List<Containers> result = new ArrayList<>();
//
//            if (request.getShipmentId() != null) {
//                log.info("Fetching shipment details for shipmentId: {}", request.getShipmentId());
...
//                log.info("Found shipmentDetails: {}", shipmentDetails);
//
//                request.setTransportMode(shipmentDetails.getTransportMode());
//                log.info("Set transport mode: {}", shipmentDetails.getTransportMode());
//
...
//                log.info("Constructed specification and pageable pair");
//
//                Page<Containers> containers = containerDao.findAll(pair.getLeft(), pair.getRight());
//                log.info("Fetched containers page with {} records", containers.getTotalElements());
//
//                List<Containers> containersList = containers.getContent();
//                log.info("Extracted containers list: {}", containersList.size());
//
//                result.addAll(containersList);
//                log.info("Added containers to result. Current result size: {}", result.size());
//            }
//
//            if (request.getConsolidationId() != null) {
//                log.info("Fetching consolidation details for consolidationId: {}", request.getConsolidationId());
...
//                log.info("Found consolidationDetails: {}", consolidationDetails);
//
//                request.setTransportMode(consolidationDetails.getTransportMode());
//                log.info("Set transport mode from consolidation: {}", consolidationDetails.getTransportMode());
//
//                request.setExport(consolidationDetails.getShipmentType() != null &&
//                        consolidationDetails.getShipmentType().equalsIgnoreCase(Constants.DIRECTION_EXP));
//                log.info("Set export flag from consolidation: {}", request.isExport());
//
//                ListCommonRequest req2 = constructListCommonRequest(Constants.CONSOLIDATION_ID, Long.valueOf(request.getConsolidationId()), "=");
//                log.info("Constructed ListCommonRequest for consolidation: {}", req2);
//
//                Pair<Specification<Containers>, Pageable> pair = fetchData(req2, Containers.class);
//                log.info("Constructed specification and pageable pair for consolidation");
//
//                Page<Containers> containers = containerDao.findAll(pair.getLeft(), pair.getRight());
//                log.info("Fetched containers page for consolidation with {} records", containers.getTotalElements());
//
//                List<Containers> containersList = containers.getContent();
//                log.info("Extracted containers list from consolidation: {}", containersList.size());
//
//                if (result.isEmpty()) {
//                    result.addAll(containersList);
//                    log.info("Result was empty. Added all consolidation containers. Result size now: {}", result.size());
//                } else {
//                    result = result.stream().filter(containersList::contains).toList();
//                    log.info("Performed intersection with consolidation containers. Result size now: {}", result.size());
//                }
//            }
//
...
//            log.info("Generated filename with timestamp: {}", filenameWithTimestamp);
//
...
//            log.info("Set response headers for Excel download");
//
//            try (XSSFWorkbook workbook = new XSSFWorkbook()) {
//                XSSFSheet sheet = workbook.createSheet("Containers");
//                log.info("Created new Excel sheet: Containers");
//
//                List<ContainersExcelModel> model = commonUtils.convertToList(result, ContainersExcelModel.class);
//                log.info("Converted container list to excel model. Model size: {}", model.size());
//
...
//                    log.info("Excel file written to response output stream");
//                }
//            }
//
//        } catch (Exception ex) {
//            log.error("Error occurred in downloadContainers: {}", ex.getMessage(), ex);
