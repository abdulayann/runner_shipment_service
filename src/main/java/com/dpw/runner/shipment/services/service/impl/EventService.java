...
public class EventService implements IEventService {
    ...
    private boolean updateActual(ShipmentDetails shipmentDetails, Container container) {
        AtomicBoolean isShipmentUpdateRequired = new AtomicBoolean(false);
        if (container == null || container.getEvents() == null) {
            log.warn("No container or events available for shipment ID {}", shipmentDetails.getShipmentId());
            return false;
        }

        List<Events> shipmentEvents = shipmentDetails.getEventsList();

        Map<String, Event> containerEventMapFromTracking = container.getEvents().stream()
                .map(event -> {
