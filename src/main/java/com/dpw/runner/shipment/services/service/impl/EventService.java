...
public class EventService implements IEventService {
    ...
    public ResponseEntity<?> create(CommonRequestModel commonRequestModel) {
        ...
        try {
            ...
            log.info("Event Details created successfully for Id {} with Request Id {}", bookingCarriage.getId(), LoggerHelper.getRequestIdFromMDC());
        } catch (Exception e) {
    ...
    public ResponseEntity<?> delete(CommonRequestModel commonRequestModel) {
        ...
        try {
            ...
            if(request.getId() == null && request.getGuid() == null) {
                log.error("Request Id and Guid is null for Event delete with Request Id {}", LoggerHelper.getRequestIdFromMDC());
            }
            if(request.getId() == null) {
                log.error("Request Id is null for Event delete with Request Id {}", LoggerHelper.getRequestIdFromMDC());
            }
            if(request.getGuid() == null) {
                log.error("GUID is null for Event delete with Request Id {}", LoggerHelper.getRequestIdFromMDC());
            }
            Optional<Events> events;
            if(request.getId() != null) {
                long id = request.getId();
                events = eventDao.findById(id);
                if (!events.isPresent()) {
                    log.debug("Event is null for Id {} with Request Id {}", id, LoggerHelper.getRequestIdFromMDC());
                    throw new DataRetrievalFailureException(DaoConstants.DAO_DATA_RETRIEVAL_FAILURE);
                }
            } else {
                UUID guid = UUID.fromString(request.getGuid());
                events = eventDao.findByGuid(guid);
                if (!events.isPresent()) {
                    log.debug("Event is null for GUId {} with Request Id {}", guid, LoggerHelper.getRequestIdFromMDC());
                    throw new DataRetrievalFailureException(DaoConstants.DAO_DATA_RETRIEVAL_FAILURE);
                }
            }
            eventDao.delete(events.get());
            log.info("Deleted Event for Id {} with Request Id {}", request.getId(), LoggerHelper.getRequestIdFromMDC());
    ...
    public ResponseEntity<?> retrieveById(CommonRequestModel commonRequestModel) {
        ...
        try {
            ...
            if(request.getId() == null && request.getGuid() == null) {
                log.error("Request Id and Guid is null for Event retrieve with Request Id {}", LoggerHelper.getRequestIdFromMDC());
            }
            if(request.getId() == null) {
                log.error("Request Id is null for Event retrieve with Request Id {}", LoggerHelper.getRequestIdFromMDC());
            }
            if(request.getGuid() == null) {
                log.error("GUID is null for Event retrieve with Request Id {}", LoggerHelper.getRequestIdFromMDC());
            }
            Optional<Events> events;
            ...
            if(request.getId() != null) {
                ...
                log.info("Event details fetched successfully for GUId {} with Request Id {}", guid, LoggerHelper.getRequestIdFromMDC());
            }
            response = convertEntityToDto(events.get());
