...
public class ConsolidationService implements IConsolidationService {
    ...
    public ResponseEntity<?> updateUtilization(CommonRequestModel commonRequestModel) {
        String responseMsg;
        try {
            ConsolidationDetailsRequest consolidationDetailsRequest = (ConsolidationDetailsRequest) commonRequestModel.getData();
            ConsolidationDetails consolidationDetails = convertToClass(consolidationDetailsRequest, ConsolidationDetails.class);
            BigDecimal consolidatedWeight = (BigDecimal) unitConversionService.convertUnit(Constants.MASS, consolidationDetails.getAchievedQuantities().getConsolidatedWeight(), consolidationDetails.getAchievedQuantities().getConsolidatedWeightUnit(), Constants.WEIGHT_UNIT_KG);
            BigDecimal weight = (BigDecimal) unitConversionService.convertUnit(Constants.MASS, consolidationDetails.getAllocations().getWeight(), consolidationDetails.getAllocations().getWeightUnit(), Constants.WEIGHT_UNIT_KG);
            ...
            BigDecimal consolidatedVolume = (BigDecimal) unitConversionService.convertUnit(Constants.VOLUME, consolidationDetails.getAchievedQuantities().getConsolidatedVolume(), consolidationDetails.getAchievedQuantities().getConsolidatedVolumeUnit(), Constants.VOLUME_UNIT_M3);
            BigDecimal volume = (BigDecimal) unitConversionService.convertUnit(Constants.VOLUME, consolidationDetails.getAllocations().getVolume(), consolidationDetails.getAllocations().getVolumeUnit(), Constants.VOLUME_UNIT_M3);
    ...
    public ResponseEntity<?> calculateAchieved_AllocatedForSameUnit(CommonRequestModel commonRequestModel) {
        ...
        try {
            ...
            if(consolidationDetails.getAchievedQuantities().getConsolidatedWeightUnit() != consolidationDetails.getAllocations().getWeightUnit()) {
                BigDecimal val = (BigDecimal) unitConversionService.convertUnit(Constants.MASS, consolidationDetails.getAchievedQuantities().getConsolidatedWeight(), consolidationDetails.getAchievedQuantities().getConsolidatedWeightUnit(), consolidationDetails.getAllocations().getWeightUnit());
            ...
            if(consolidationDetails.getAchievedQuantities().getConsolidatedVolumeUnit() != consolidationDetails.getAllocations().getVolumeUnit()) {
                BigDecimal val = (BigDecimal) unitConversionService.convertUnit(Constants.VOLUME, consolidationDetails.getAchievedQuantities().getConsolidatedVolume(), consolidationDetails.getAchievedQuantities().getConsolidatedVolumeUnit(), consolidationDetails.getAllocations().getVolumeUnit());
    ...
    public ResponseEntity<?> calculateChargeable(CommonRequestModel commonRequestModel) {
        ...
        try {
            ...
            if(!weightUnit.isEmpty() && !volumeUnit.isEmpty() && (transportMode != Constants.TRANSPORT_MODE_SEA || consolidationDetails.getShipmentType() != Constants.SHIPMENT_TYPE_LCL)) {
            ...
            if(!weightUnit.isEmpty() && !volumeUnit.isEmpty() && transportMode == Constants.TRANSPORT_MODE_SEA && consolidationDetails.getShipmentType() == Constants.SHIPMENT_TYPE_LCL) {
                volume = (BigDecimal) unitConversionService.convertUnit(Constants.VOLUME, volume, volumeUnit, Constants.VOLUME_UNIT_M3);
                weight = (BigDecimal) unitConversionService.convertUnit(Constants.MASS, weight, weightUnit, Constants.WEIGHT_UNIT_KG);
    ...
    public ConsolidationDetails setChargeableValues(ConsolidationDetails consolidationDetails, String transportMode, String weightUnit, String volumeUnit, BigDecimal weight, BigDecimal volume) throws Exception{
        String responseMsg;
        try {
            if(!weightUnit.isEmpty() && !volumeUnit.isEmpty() && !transportMode.isEmpty()) {
                switch (transportMode) {
                    ...
                    case Constants.TRANSPORT_MODE_FSA:
                        BigDecimal volInM3 = (BigDecimal) unitConversionService.convertUnit(Constants.VOLUME, volume, volumeUnit, Constants.VOLUME_UNIT_M3);
                    ...
                    case Constants.TRANSPORT_MODE_ROA:
                        BigDecimal wtInKG = (BigDecimal) unitConversionService.convertUnit(Constants.MASS, weight, weightUnit, Constants.WEIGHT_UNIT_KG);
                        BigDecimal vlInM3 = (BigDecimal) unitConversionService.convertUnit(Constants.VOLUME, volume, volumeUnit, Constants.VOLUME_UNIT_M3);
