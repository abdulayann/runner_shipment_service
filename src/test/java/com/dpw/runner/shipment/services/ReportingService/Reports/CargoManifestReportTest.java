...
class CargoManifestReportTest extends CommonMocks {
    ...
    void testCsdInfoEmpty() {
        ...
        // When
        cargoManifestReport.checkCsdInfo(cargoInfoRows, cargoManifestModel, dictionary, v1TenantSettingsResponse);
    ...
    void testCsdInfoNotEmptyWithDateTime() {
        CargoManifestReport cargoManifestReport = spy(CargoManifestReport.class);

        // Create mocks for the required classes
        Awb awb = mock(Awb.class);
        CargoManifestModel cargoManifestModel = new CargoManifestModel();
        cargoManifestModel.awb = awb;
        ...
        // Mock the ConvertToDPWDateFormat method
        when(cargoManifestReport.ConvertToDPWDateFormat(any(), anyString(), anyBoolean())).thenReturn("09/05/2024");
        ...
        // Verify the result
        String expectedDateFormat = cargoManifestReport.ConvertToDPWDateFormat(now, "MM/dd/yyyy", true);
    ...
    @Test
    void testCsdInfoNotEmptyWithNullDateTime1() {
        // Arrange
        CargoManifestReport cargoManifestReport1 = spy(CargoManifestReport.class);
        CargoManifestModel cargoManifestModel = new CargoManifestModel();
        Awb awb = new Awb();
        cargoManifestModel.awb = awb;

        // Given
        cargoInfoRows = mock(AwbCargoInfo.class);
        when(cargoInfoRows.getCsdInfo()).thenReturn("SomeInfo");
        awb.setOriginalPrintedAt(null);  // Set directly

        Map<String, Object> dictionary = new HashMap<>();

        // Act
        cargoManifestReport1.checkCsdInfo(cargoInfoRows, cargoManifestModel, dictionary, v1TenantSettingsResponse);
