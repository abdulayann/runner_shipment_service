...
class MasterDataUtilsTest extends CommonMocks {
    ...
    @Test
    void getTenantDataFromCache() {
        var response = masterDataUtils.getTenantDataFromCache(null);
        assertNotNull(response);
        assertTrue(response.isEmpty());
    }

    @Test
    void getTenantDataFromCache2() {
        Cache cache = mock(Cache.class);
        when(cacheManager.getCache(anyString())).thenReturn(cache);
        var response = masterDataUtils.getTenantDataFromCache(Set.of());
        assertNotNull(response);
        assertTrue(response.isEmpty());
    }

    @Test
    void getTenantDataFromCache3() {
        MasterDataUtils spyService = spy(masterDataUtils);
        Cache cache = mock(Cache.class);
        TenantModel tenantModel =new TenantModel();
        tenantModel.setTenantId(728L);
        tenantModel.setTenantName("Test Tenant");
        lenient().when(jsonHelper.convertValueToList(any(), eq(TenantModel.class))).thenReturn(List.of(tenantModel));
        when(v1Service.listCousinBranches(any())).thenReturn(V1DataResponse.builder().build());
        when(cacheManager.getCache(anyString())).thenReturn(cache);
        var response = spyService.getTenantDataFromCache(Set.of("728"));
        assertNotNull(response);
    }
